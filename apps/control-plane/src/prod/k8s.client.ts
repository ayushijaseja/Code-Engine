...
export class K8sClient {
    ...
    static async applyIngress(namespace: string, ingressSpec: k8s.V1Ingress) {
        const ingressName = ingressSpec.metadata!.name!;
        try {
            await k8sNetworkingApi.createNamespacedIngress(namespace, ingressSpec);
            console.log(`[K8s] Ingress ${ingressName} created.`);
        } catch (err: any) {
            if (err.body?.reason !== 'AlreadyExists' && err.statusCode !== 409) throw err;
            console.log(`[K8s] Ingress ${ingressName} exists.`);
        }
    }

    static async deleteIngress(name: string, namespace: string) {
        try {
            await k8sNetworkingApi.deleteNamespacedIngress(name, namespace);
            console.log(`[K8s] Deleted Ingress: ${name}`);
        } catch (e: any) {
            if (e.statusCode !== 404) console.error(`[K8s Error] Ingress deletion failed:`, e.body?.message);
        }
    }
}